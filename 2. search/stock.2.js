...
async function printConsole(content, code) {
    //console.log(content)
...
function startData(list) {
    (async () => {
        ...
        var i = 0;
        for(const li of list){
            await page.goto("https://fchart.stock.naver.com/sise.nhn?symbol=" + li.code + "&timeframe=day&count=300&requestType=0", { waitUntil: "networkidle2" });
            const content = await page.content();
            
            await printConsole(content, li.code);
